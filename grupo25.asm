...
move_objeto:
  ...
  MOVB  R0, [R3]  ; R0 <- Linha inicial do objeto (Byte)
  ADD   R3, 1     ; próximo byte
  MOVB  R1, [R3]  ; R1 <- Coluna inicial do objeto (Byte)
  ADD   R3, 1     ; próximo byte 
  MOVB  R2, [R3]  ; Direção de movimento horizontal (Byte)
  ...
  ADD   R0, 1     ; Objeto desce uma linha
  ADD   R1, R2    ; Objeto vai na sua respetiva direção (esquerda, baixo, direita)

  SUB   R3, 4     ; Endereço da linha
  MOVB  [R3], R0
  ADD   R3, 1     ; Endereço da coluna
  MOVB  [R3], R1
