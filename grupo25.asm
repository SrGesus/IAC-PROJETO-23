...
; *****************************************************************************
  ...
  SOM_NAVE_EXPLOSAO       EQU 3
  SOM_BOTAO               EQU 4
  SOM_GAMEOVER_ENERGIA    EQU 5
...
LISTA_ROTINAS:
  WORD LISTA_ROTINAS_INICIO ; Lista rotinas default
...
decrementa_energia:
  ...
  SUB   R3, 3
  MOV   [energia],  R3
  MOV   [atualiza_display],  R3 ; Valor irrelevante
...
nave_sem_energia:
  ...
  MOV [SEL_CEN_FUNDO], R1
  MOV R1, SOM_GAMEOVER_ENERGIA
  MOV [TOCA_SOM], R1

  ; Desativa outros controles do teclado
  MOV R1, LISTA_ROTINAS_INICIO
...
pausa:
  MOV R0, FUNDO_PAUSA
  MOV [SEL_CEN_FRONTAL], R0
  MOV R0, LISTA_ROTINAS_PAUSA
  MOV [LISTA_ROTINAS], R0
  MOV R0, 0
  MOV [estado_jogo], R0
  RET
...
verifica_colisão_asteroide:
  ...
  CMP   R1, 0
  JLE    sair_verifica_colisão_asteroide
